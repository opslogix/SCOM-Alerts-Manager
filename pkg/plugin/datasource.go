...
func (d *ScomDatasource) handleQuery(query backend.DataQuery) (data.Frames, error) {
	...
	backend.Logger.Info("Received query", "json", string(query.JSON))

	scomQuery, err := ParseQuery(query.JSON)
	if err != nil {
		backend.Logger.Error("Failed to parse query", "error", err, "json", string(query.JSON))
	...
	case models.AlertQuery:
		{
			...
			backend.Logger.Info("Fetching alerts with criteria", "criteria", criteria)
			alerts, err := d.client.GetAlerts(criteria)
			if err != nil {
				backend.Logger.Error("Failed to get alerts", "error", err)
	...
	case models.PerformanceQuery:
		{
			return nil, fmt.Errorf("performance queries are no longer supported")
		}
	case models.StateQuery:
		{
			return nil, fmt.Errorf("state queries are no longer supported")
		}
	}
...
func ParseQuery(jsonData []byte) (interface{}, error) {
	...
	if err := json.Unmarshal(jsonData, &base); err != nil {
		backend.Logger.Error("Failed to unmarshal base query", "error", err)
		return nil, err
	}

	backend.Logger.Info("Parsed query type", "type", base.Type)
	...
	case "alerts":
		...
		backend.Logger.Info("Parsed alert query", "criteria", q.Criteria)
		return q, nil
	case "performance":
